...
void ShowBMP(char *file, SDL_Surface *screen, int x, int y){
	...
	SDL_SetColorKey(image, SDL_SRCCOLORKEY, SDL_MapRGB(image->format,255,0,255));//255,0,255
...
int maxanswerboxlength = 0;
	...
	letterBankRect.w = 10;
	letterBankRect.h = 20;//14 probably use to pull from image
	letterBankRect.y = 0;
	letterBankRect.x = 0; // letter is chosen by 10*letter where a is 0
		...
		if (numWords == 12){
			numWords = 0;
			acrossOffset += (listLetters * 15) + 4; //originally *18 +9 
			outerrect.y = 295;
			outerrect.x = acrossOffset;
		}
		else{
			outerrect.x = acrossOffset;
			outerrect.y += 15;//originally 16
		}
		current=current->next;
	}
...
int maxanswerboxlength = 0;
	...
	while (!happy || !happier){//
		...
		// generate anagrams from random wordNew Game Clicked
		ag(&(*head), dlbHead, &guess, &remain);

		sort(&(*head));
		answersSought = Length(*head);
		happy = ((answersSought <= 48) && (answersSought >= 6)); //originally 40=77
		current = *head;
			...
			if (numWords == 12){
				numWords = 0;
				acrossOffset += (listLetters * 15) + 4; 
			}
			current=current->next;
		}
		printf("Num of Answer %i\n", answersSought);
		if (numWords == 0){
			maxanswerboxlength=acrossOffset;
			//printf("numWord = 0\n");		
		}
		else {
			maxanswerboxlength=acrossOffset+(listLetters *15);
			//printf("numWord != 0\n");
		}
		
		happier = maxanswerboxlength <= 320;
				
		printf("max answer box length in new game %i\n", maxanswerboxlength);		
		/*printf("happy %i\n", happy);		
		printf("happier %i\n", happier);
		*/
		...
		if(!happier){
			printf("AnswerBoxes Exceeds Display %i\n", maxanswerboxlength);
		}
	}
...
//pthread_t audio;
	...
	screen=SDL_SetVideoMode(0,0,16,0);//|SDL_FULLSCREEN);
	if (screen == NULL)
	{
	...
	letterBank = SDL_LoadBMP("images/letterBank.bmp"); //  IMG_Load("images/letterBank.png"); 
	smallLetterBank = IMG_Load("images/smallLetterBank.png"); //SDL_LoadBMP("images/smallLetterBank.bmp");
	numberBank = SDL_LoadBMP("images/numberBank.bmp"); // IMG_Load("images/numberBank.png");
