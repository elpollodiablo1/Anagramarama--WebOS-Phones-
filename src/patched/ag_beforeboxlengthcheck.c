...
int maxanswerboxlength = 0;
	...
	//maxanswerboxlength=acrossOffset+(listLetters *16);
	//printf("max answer box length %i\n", maxanswerboxlength); //added by me
...
int maxanswerboxlength = 0;
	...
	while (!happy || !happier){
		...
		happy = ((answersSought <= 40) && (answersSought >= 6)); //originally 40=77
			...
			if (numWords == 11){
				numWords = 0;
				acrossOffset += (listLetters * 16) + 5; 
			}
			current=current->next;
		}

		maxanswerboxlength=acrossOffset+(listLetters *16);
		happier = maxanswerboxlength <= 310;
		printf("max answer box length %i\n", maxanswerboxlength);
