...
void bufferSounds(struct sound** soundCache){
	...
	pushSound(&(*soundCache),"click-answer", "/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/audio/click-answer.wav");
	pushSound(&(*soundCache),"click-shuffle", "/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/audio/click-shuffle.wav");
	pushSound(&(*soundCache),"foundbig", "/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/audio/foundbig.wav");
	pushSound(&(*soundCache),"found", "/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/audio/found.wav");
	pushSound(&(*soundCache),"clear", "/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/audio/clearword.wav");
	pushSound(&(*soundCache),"duplicate", "/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/audio/duplicate.wav");
	pushSound(&(*soundCache),"badword", "/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/audio/badword.wav");
	pushSound(&(*soundCache),"shuffle", "/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/audio/shuffle.wav");
	pushSound(&(*soundCache),"clock-tick", "/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/audio/clock-tick.wav");
...
int listLetters = 0;
	...
	letterBankRect.x = 0; // letter is chosen by 10*letter where a is 0
		...
		for (i=0;i<current->length;i++){
			...
			if (current->guessed){
				SDL_FillRect(screen, &innerrect,SDL_MapRGB(screen->format,255,255,255)); // background color once guessed
			}
			else{
				SDL_FillRect(screen, &innerrect,SDL_MapRGB(screen->format,217,220,255)); //217,220,255 color of blank box
			}

			if (current->found){
				innerrect.x+=2;
				letterBankRect.x = 10 * ((int)current->anagram[i] - 97);
				SDL_SetColorKey(smallLetterBank, SDL_SRCCOLORKEY, SDL_MapRGB(smallLetterBank->format,255,0,255));//255,0,255
		...
		if (numWords == 11){
			numWords = 0;
			acrossOffset += (listLetters * 16) + 5; //originally *18 +9 
			outerrect.y = 295;
			outerrect.x = acrossOffset;
		}
		else{
			outerrect.x = acrossOffset;
			outerrect.y += 16;//originally 19
		}

		current=current->next;
	}
printf("max answer row %i\n", acrossOffset+(listLetters *16)); //added by me
}
...
int i, len;
int foundWord = 0;
char test[len]; //originally char test[7]
...
//	char test[len];			//bugfix by Adolfo - what was I thinking!  and why does it only fail on RH 7.3??
	...
	while (current != NULL){
		...
		if (!strcmp(current->anagram, test)){
			//printf("Before foundWord\n");
			foundWord = 1;
			//printf("foundword Set\n");
			if (!current->found){
				score+=current->length;
				totalScore+=current->length;
				answersGot++;
				//printf("answer Got increased\n");
				if (len==bigWordLen){
					gotBigWord = 1;
...
SDL_Rect fromrect, torect, blankRect;
	...
	for (i = 0; i < strlen(buffer); i++){
		...
		//SDL_SetColorKey(numberBank,SDL_SRCCOLORKEY,SDL_MapRGB(numberBank->format,255,255,255));//Added by me
		SDL_BlitSurface(numberBank, &fromrect, scoreSprite->sprite, &torect);//Displays updated score
	}
}
...
int done = 0;
	...
	wordlist=fopen("/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/wordlist.txt","r");
	...
	for (i=0;i<=filelocation;i++){
		...
		if(fscanf(wordlist, "%s", wordFromList) != EOF){
			...
			fclose(wordlist);
			fopen("/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/wordlist.txt", "r");
		}
	}
...
int index = 0;
	...
	for (i=0;i<len;i++){
		...
		if(shuffle[i] != ASCII_SPACE && shuffle[i] != SPACE_CHAR){
			...
			SDL_SetColorKey(letterBank,SDL_SRCCOLORKEY,SDL_MapRGB(letterBank->format,255,255,255));//Added by me
...
//		printf("i:%i\n", CLOCK_WIDTH * i);
		...
		//SDL_SetColorKey(numberBank,SDL_SRCCOLORKEY,SDL_MapRGB(numberBank->format,255,255,255));//Added by me
		SDL_BlitSurface(numberBank, &fromrect, thisLetter->sprite, &torect); //original 
		SDL_BlitSurface(numberBank, &blankRect, thisLetter->replace, &torect);//original add big white rectangel
	}
...
int i;
	...
	if(screen->flags & SDL_SWSURFACE)
			flags |= SDL_SWSURFACE;
	//if(screen->flags & SDL_HWSURFACE) //removed by me
	//		flags |= SDL_HWSURFACE; //removed by me
	...
	for (i=0;i<5;i++){
		...
		//SDL_SetColorKey(numberBank,SDL_SRCCOLORKEY,SDL_MapRGB(numberBank->format,0,0,0));//Added by me
		SDL_BlitSurface(numberBank, &fromrect, thisLetter->sprite, &torect);// orginal
		SDL_BlitSurface(numberBank, &blankRect, thisLetter->replace, &torect);//original
	}
...
synopsis: do all of the initialisation for a new game:
	  ...
	  (must get less than 40 anagrams to display on screen)
...
int i;
	...
	// show background
	ShowBMP("/media/cryptofs/apps/usr/palm/applications/com.cribme.ag/images/background.bmp",screen, 0,0);
	...
	while (!happy){
		...
		happy = ((answersSought <= 40) && (answersSought >= 6)); //originally 40=77
		//printf("answer box length %i", acrossOffset);
		//happier = (acrossOffset < 300)
		if(!happy){
